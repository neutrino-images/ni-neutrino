...
// bind to it.
typedef httpd::compat::CNeutrinoAPI CNeutrinoAPI;
...
// neutrinoapi.h declares both of these inside httpd::compat, so the
// definitions have to sit in that namespace too: left at file scope, as the
// unnamespaced original had them, either symbol collides with the same name
// the old nhttpd copy still defines at file scope in the same program.
namespace httpd
{
namespace compat
{
...
bool CNeutrinoAPI::GetStreamInfo(int bitInfo[10])
{
	...
	if (fd == NULL)
	{
		// The removed server's debug print stood here and never ran on a box
		// as shipped: it was behind that server's debug switch, which was off
		// unless its configuration turned it on. The caller reads the false.
		return false;
	}
