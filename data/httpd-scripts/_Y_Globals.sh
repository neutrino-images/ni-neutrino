...
# The old server's file, still named because a box upgraded from an older
# image has one and the y-web era subcommands below read it. Nothing
# installs it any more.
y_config_nhttpd="$y_path_config/nhttpd.conf"
# The file the server that actually answers reads.
y_config_niweb="$y_path_config/ni-web.conf"
