...
getLanguage()
{
	# The box's own language and not the old server's. Language.selected named
	# which translation those pages drew themselves in; with the pages gone the
	# key is gone too, and this answered empty, so both labels below came out
	# English whatever the box was set to.
	#
	# The value is the name of a file in the locale directory, so it is lower
	# case: deutsch, english. The old key spelled it Deutsch, which is why the
	# two comparisons below changed with it.
	Y_LANG=$(cat %(CONFIGDIR)/neutrino.conf | grep "^language=" | cut -d= -f2)
}
