...
call_webserver()
{
	# The port of the server that answers, which is ni-web. This read the old
	# server's file, and that file stopped being installed when that server
	# left; the port then came out empty and every call here went to
	# "http://localhost:/..." and failed with nothing said.
	port=`sed -n /^port=/p $y_config_niweb | sed -e s/^port=//1`
